...
friction_raw <- raster(paste0(dir,"/data-raw/geo-raw/raster/friction.tif"))
...
scale_fill_manual(values=c("#ff6f69","#283655","#8874a3","#fe8a71",alpha=0.1))+
...
# write binary raster
# writeRaster(iflstatus,paste0(dir, "/data-raw/geo-raw/raster/ifl_bin.tif"))
# writeRaster(wdpastatus,paste0(dir, "/data-raw/geo-raw/raster/wdpa_bin.tif"))

# Hansen's forest loss data: Downloading, merging (in QGIS) and aggregating  (in R). Merging in QGIS
```

# Raw data processing
```{r reprojecting rasters}
# acesss
# projectRaster(access_raw, hum_pop_00,0.008333, projection, method="bilinear", alignOnly=FALSE, over=FALSE, paste0(dir,"/covariates/predictors/access_prj.tif")) 
# access_prj <- raster(paste0(dir,"/covariates/predictors/access_prj.tif"));plot(access_prj)

# friction
# projectRaster(friction_raw, hum_pop_00,0.008333, projection, method="bilinear", alignOnly=FALSE, over=FALSE, paste0(dir,"/covariates/predictors/friction_prj.tif"))
# friction_prj <- raster(paste0(dir,"/covariates/predictors/friction_prj.tif"));plot(friction_prj)
# mask ocean values
# friction_prj_mask <- raster(paste0(dir,"/covariates/predictors/friction_prj_mask.tif"));plot(friction_prj_mask)
# landmass <- raster(paste0(dir,"/covariates/predictors/landmass_prj.tif"));plot(landmass)
# friction_prj_maskf <-  raster::mask(x = friction_prj_mask, mask = landmass)
# plot(friction_prj_maskf)
# writeRaster(friction_prj_maskf,paste0(dir, "/covariates/predictors/friction_prj_maskf.tif"))

# humfoot_print
# projectRaster(humfoot_print_raw, hum_pop_00,0.008333, projection, method="bilinear", alignOnly=FALSE, over=FALSE, paste0(dir,"/covariates/predictors/humfoot_print_prj.tif")) 
# humfoot_print_prj <- raster(paste0(dir,"/covariates/predictors/humfoot_print_prj.tif"));plot(humfoot_print_prj)
# humfoot_print_prj_mask <- raster(paste0(dir,"/covariates/predictors/humfoot_print_prj_mask.tif"))
# plot(humfoot_print_prj_mask)
# landmass <- raster(paste0(dir,"/covariates/predictors/landmass_prj.tif"));plot(landmass)
# humfoot_print_prj_maskf <-  raster::mask(x = humfoot_print_prj_mask, mask = landmass)
# plot(humfoot_print_prj_maskf)
# writeRaster(humfoot_print_prj_maskf,paste0(dir, "/covariates/predictors/humfoot_print_prj_maskf.tif"))

# humfoot_print_prj_maskf[humfoot_print_prj_maskf > 100] <- NA
# plot(humfoot_print_prj_maskf,main = "The Raster Mask",col = c("green"),legend = FALSE,axes = FALSE,box = FALSE)
# humfoot_print_prj_maskf_lakes <-  raster::mask(x = humfoot_print_prj_maskf, mask = humfoot_print_prj_maskf)
# plot(humfoot_print_prj_maskf_lakes)
# writeRaster(humfoot_print_prj_maskf_lakes,paste0(dir, "/covariates/predictors/humfoot_print_prj_maskf_lakes.tif"))

# hum_mod
# projectRaster(hum_mod_raw, hum_pop_00,0.008333, projection, method="bilinear", alignOnly=FALSE, over=FALSE, paste0(dir,"/covariates/predictors/hum_mod_prj.tif")) 
# hum_mod_prj <- raster(paste0(dir,"/covariates/predictors/hum_mod_prj.tif"));plot(hum_mod_prj)
# mask ocean values
# hum_mod_prj_mask <- raster(paste0(dir,"/covariates/predictors/hum_mod_prj_maskf.tif"));plot(hum_mod_prj)
# world_landmass <- raster(paste0(dir,"/covariates/predictors/landmass_prj.tif"));world_landmass
# hum_mod_prj_maskf <-  raster::mask(x = hum_mod_prj_mask, mask = world_landmass)
# plot(hum_mod_prj_maskf)
# writeRaster(hum_mod_prj_maskf,paste0(dir, "/covariates/predictors/hum_mod_prj_maskf.tif"))

# forest loss
# projectRaster(for_loss_raw, hum_pop_00_raw,0.008333, projection, method="ngb", alignOnly=FALSE, over=FALSE, paste0(dir,"/covariates/predictors/for_loss_JN_prj.tif")) 
# for_loss_prj <- raster(paste0(dir,"/covariates/predictors/for_loss_JN_prj.tif"));plot(for_loss_prj)
# masking
# landmass <- raster(paste0(dir,"/covariates/predictors/landmass_prj.tif"));plot(landmass)
# for_loss_JN_prj_maskf <-  raster::mask(x = for_loss_prj, mask = landmass)
# plot(for_loss_JN_prj_maskf)
# save as INT2U 
#writeRaster(for_loss_JN_prj_maskf/1000,paste0(dir, "/covariates/predictors/for_loss_JN_prj_maskf_prop.tif"))
# Reclassifying to classify the rasta into two classess
# for_loss_JN_prj_maskf <- raster(paste0(dir,"/covariates/predictors/for_loss_JN_prj_maskf.tif"));plot(for_loss_JN_prj_maskf)

# threat_species
# projectRaster(threat_species_raw, hum_pop_00,0.008333, projection, method="bilinear", alignOnly=FALSE, over=FALSE, paste0(dir,"/covariates/predictors/threat_species_prj.tif")) 
# threat_species_prj <- raster(paste0(dir,"/covariates/predictors/threat_species_prj.tif"));plot(threat_species_prj)

# cum_hum_impact
# projectRaster(cum_hum_impact_raw, hum_pop_00_raw,0.008333, projection, method="bilinear", alignOnly=FALSE, over=FALSE, paste0(dir,"/covariates/predictors/cum_hum_impact_prj.tif")) 
# cum_hum_impact_prj <- raster(paste0(dir,"/covariates/predictors/cum_hum_impact_prj.tif"));plot(cum_hum_impact_prj)
# minValue(cum_hum_impact_prj);maxValue(cum_hum_impact_prj)

# sp_rich
# projectRaster(sp_rich_raw, hum_pop_00,0.008333, projection, method="bilinear", alignOnly=FALSE, over=FALSE, paste0(dir,"/covariates/predictors/sp_rich_prj.tif")) 
# sp_rich_prj <- raster(paste0(dir,"/covariates/predictors/sp_rich_prj.tif"));plot(sp_rich_prj)

# sp_datadef
# projectRaster(sp_datadef_raw, hum_pop_00,0.008333, projection, method="bilinear", alignOnly=FALSE, over=FALSE, paste0(dir,"/covariates/predictors/sp_datadef.tif")) 
# sp_datadef_prj <- raster(paste0(dir,"/covariates/predictors/sp_datadef_prj.tif"));plot(sp_datadef_prj)

# wdpa
# projectRaster(wdpa_raw, hum_pop_00_raw,0.008333, projection, method="ngb", alignOnly=FALSE, over=FALSE, paste0(dir,"/covariates/predictors/wdpa_bin_prj.tif")) 
# wdpa_prj <- raster(paste0(dir,"/covariates/predictors/wdpa_bin_prj.tif"));plot(wdpa_prj)

# ifl
# projectRaster(ifl_raw, hum_pop_00_raw,0.008333, projection, method="ngb", alignOnly=FALSE, over=FALSE, paste0(dir,"/covariates/predictors/ifl_bin_prj.tif")) 
# ifl_prj <- raster(paste0(dir,"/covariates/predictors/ifl_bin_prj.tif"));plot(ifl_prj)

# biomes
# projectRaster(biomesNum_raw, hum_pop_00_raw,0.008333, projection, method="ngb", alignOnly=FALSE, over=FALSE, paste0(dir,"/covariates/predictors/biomesNum_prj.tif")) 
# biomesNum_prj <- raster(paste0(dir,"/covariates/predictors/biomesNUm_prj.tif"));plot(biomesNUm_prj)

# elevation
# projectRaster(elevation_raw, hum_pop_00,0.008333, projection, method="bilinear", alignOnly=FALSE, over=FALSE, paste0(dir,"/covariates/predictors/elevation_prj.tif")) 
# elevation_prj <- raster(paste0(dir,"/covariates/predictors/elevation_prj.tif"));plot(elevation_prj)
# mask ocean values
# elevation_prj_maskf <- raster(paste0(dir,"/covariates/predictors/elevation_prj_mask.tif"));plot(elevation_prj)
# world_landmass <- raster(paste0(dir,"/covariates/predictors/landmass_prj.tif"));world_landmass
# elevation_prj_maskf <-  raster::mask(x = elevation_prj_maskf, mask = world_landmass)
# plot(elevation_prj_maskf)
# writeRaster(elevation_prj_maskf,paste0(dir, "/covariates/predictors/elevation_prj_maskf.tif"))

# tri
# projectRaster(tri_raw, hum_pop_00_raw,0.008333, projection, method="bilinear", alignOnly=FALSE, over=FALSE, paste0(dir,"/covariates/predictors/tri_prj.tif")) 
# tri_prj <- raster(paste0(dir,"/covariates/predictors/tri_prj.tif"));plot(tri_prj)

# extent <- extent(-180, 180, -90, 90)
# prj <- "+proj=longlat +ellps=WGS84 +datum=WGS84 +no_defs"
# extent(elevation_prj_maskf) <- extent(extent)
# crs(elevation_prj_maskf) <-crs(prj)
# extract tri from elevation 
# tri_prj_mask <- terrain(elevation_prj_maskf, opt="TRI")#opt can also be slope, aspect, TPI, roughness and flowdir
# plot(tri_prj_mask)
# writeRaster(tri_prj_mask, paste0(dir, "/covariates/predictors/tri_prj_mask.tif"))

# income
projectRaster(income_raw, hum_pop_00_raw,0.008333, projection, method="ngb", alignOnly=FALSE, over=FALSE, paste0(dir,"/covariates/predictors/income_Avgp_prj_prj.tif"))
# income_prj <- raster(paste0(dir,"/covariates/predictors/income_prj.tif"));plot(income_prj)

# landmass
# projectRaster(landmass_raw, hum_pop_00_raw,0.008333, projection, method="ngb", alignOnly=FALSE, over=FALSE, paste0(dir,"/covariates/predictors/landmass_prj.tif"))
# landmass <- raster(paste0(dir,"/covariates/predictors/landmass_prj.tif"));plot(landmass)
```
