...
# Setup: projections and functions
...
# Setup: setting working directory
```{r working directory}
...
# Data: shapefiles,.csv tables, and rasters
```{r}
# shape files
...
# Locations for MaxEnt modelling
...
# AUC Jacknife tests
AUCjktglobal <- read.csv(paste0(dir,"/output/models/global/global_AUCjkt.csv"))
AUCjktbiome1 <- read.csv(paste0(dir,"/output/models/biome1/biome1_AUCjkt.csv"))
AUCjktbiome7 <- read.csv(paste0(dir,"/output/models/biome7/biome7_AUCjkt.csv"))
AUCjktbiome12 <- read.csv(paste0(dir,"/output/models/biome12/biome12_AUCjkt.csv"))
AUCjktbiome4 <- read.csv(paste0(dir,"/output/models/biome4/biome4_AUCjkt.csv"))

# Model Permutation Importance
PI_global <- read.csv(paste0(dir,"/output/models/global/global_PI.csv"))
PI_biome1 <- read.csv(paste0(dir,"/output/models/biome1/biome1_PI.csv"))
PI_biome7 <- read.csv(paste0(dir,"/output/models/biome7/biome7_PI.csv"))
PI_biome12 <- read.csv(paste0(dir,"/output/models/biome12/biome12_PI.csv"))
PI_biome4 <- read.csv(paste0(dir,"/output/models/biome4/biome4_PI.csv"))

# Percent of locations in the different disparity quintiles
Biome_disparity <- read.csv(paste0(dir,"/output/geo-proc/biv-maps/biodcrisis/percent_disparity.csv"))
```

# Figure S1 (correlation matrix: Testing collinearity among predictors)
...
# Figure S2 (Visualising distribution of avialable vs. locations)
```{r Figure S2: data distribution between available vs. locations }
...
continuous <- ggarrange(sp_rich_graph,num_sp_impacted_graph,access_graph,forloss_graph,tri_graph,elevation_graph, ncol=2, nrow=3);continuous
...
categorical_graphs <- biomes_graph + wdpa_graph + income_graph + ifl_graph
categorical_graphs
...
# create raster stacks for different models; global, biome 1, 7, 12 and 4
...
# Figure S3
```{r Figure S3: Response curves for the predictors}
...
# Figure 4
```{r Figure 4: Four biome prediction maps}
...
# Figure S4
```{r Figure S4: Jacknife tests (lollipop plots)}
# AUC tests at global scale; global, biome 1, biome 4, biome 7 and biome 12
p4#global(a)
p5#biome1(b)
p7#biome7(c)
...
# Creating a Defaunation risk map: Biome 1 (Tropical & Subtropical Moist Broadleaf Forests)
```{r biome 1 Defaunation risk map}
...
# create Defaunation risk map covariate raster stack
...
biodcrisis_map_biome1 <- weighted.mean(x = biodcrisis_covs_norm_biome1, w = weights)
plot(biodcrisis_map_biome1)

# write the weighted raster as the “defaunation risk map"
writeRaster(biodcrisis_covs_norm_biome1,filename = paste0(dir,"/output/geo-proc/biodcrisis-maps/Biome1biodcrisis_map.tif"))

# load in the Biome1 Defaunation risk map
biodcrisisbiome1_map_ras <- raster(paste0(dir,"/output/geo-proc/biodcrisis-maps/Biome1biodcrisis_map_FinalEEExtMatch3.tif"))
...
# Creating a Defaunation risk map: Biome 7 (Tropical & Subtropical Grasslands, Savannah and Shrubs)
```{r biome 7 Defaunation risk map}
# normalize covariates
...
# calculate weighted mean on the stack
biodcrisis_map_biome7 <- weighted.mean(x = biodcrisis_covs_norm_biome7, w = weights)
plot(biodcrisis_map_biome7)

writeRaster(biodcrisis_covs_norm_biome7,filename = paste0(dir,"/output/geo-proc/biodcrisis-maps/Biome7biodcrisis_map.tif"))

# load in the Biome7 Defaunation risk map
...
# Creating a Defaunation risk map: Biome 12 (Mediterranean Forests, Woodlands & Scrub)
```{r biome 12 Defaunation risk map}
# normalize covariates
...
# calculate weighted mean on the stack
biodcrisis_map_biome12 <- weighted.mean(x = biodcrisis_covs_norm_biome12, w = weights)
plot(biodcrisis_map_biome12)

writeRaster(biodcrisis_covs_norm_biome12,filename = paste0(dir,"/output/geo-proc/biodcrisis-maps/Biome12biodcrisis_map.tif"))

# load in the Biome12 Defaunation risk map
...
# Creating a Defaunation risk map: Biome 4 (Temperate Broadleaf & Mixed Forests)
```{r biome 4 Defaunation risk map}
# normalize covariates
...
# calculate weighted mean on the stack
biodcrisis_map_biome4 <- weighted.mean(x = biodcrisis_covs_norm_biome4, w = weights)
plot(biodcrisis_map_biome4)

writeRaster(biodcrisis_covs_norm_biome4,filename = paste0(dir,"/output/geo-proc/biodcrisis-maps/Biome4biodcrisis_map.tif"))

# load in the Biome4 Defaunation risk map
...
# Figure S5
```{r Figure S5: Defaunation risk maps}
...
# Creating a bivariate map: Biome 1 (Tropical & Subtropical Moist Broadleaf Forests)
...
# Creating a bivariate map: Biome 7 (Tropical & Subtropical Grasslands, Savannah and Shrubs)
...
# Creating a bivariate map: Biome 12 (Mediterranean Forests, Woodlands & Scrub)
...
# Creating a bivariate map: Biome 4 (Temperate Broadleaf & Mixed Forests)
...
# Figure 5
```{r Figure 5: Defaunation risk disparity maps}
# disparity
...
# Percent of locations in the different Defaunation risk map quintiles for all biomes
# as a matrix
dataframe <- as.data.frame.matrix(Biome_disparity)

# the plot
biomes_biodcrisis_quintiles <- ggplot(data=dataframe, aes(x=Biomes,y=percent, fill= factor(Quintiles))) +
geom_bar(stat="identity",color="white", alpha=0.95,position = position_fill(reverse = TRUE))+
labs(fill="Quintiles")+
xlab("Most researched biomes")+ylab("Proportion of locations")+
theme_bw()+
...
# SessionInfo ## DO NOT REMOVE!
```{r sessionInfo}
## We store the settings of your computer and the current versions of the packages used to allow for reproducibility
Sys.time()
#git2r::repository()
sessionInfo()
```
